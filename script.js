...
const fullJobList = jobData.map(job => {

    return `<li class="job-listing">
          ...
          <div class="job-listing__skills">
            ...
            ${job.languages.map(lang => `<li class='job-listing__skill'> ${lang} </li>`).join('')}
            ${job.tools.map(tool => `<li class='job-listing__skill'> ${tool} </li>`).join('')}
            </ul>
          </div>
        </li>`
}).join('');
console.log(fullJobList);

const jobListingsList = document.querySelector('.job-listings-container');
jobListingsList.insertAdjacentHTML('afterbegin', fullJobList);


//function filter list -- filter out items that meet given conditions
